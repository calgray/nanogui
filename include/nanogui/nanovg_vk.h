...
static int vknvg_renderCreate(void *uptr) {
  ...
  uint32_t* fillVertShader = nullptr; uint32_t fillVertShaderSize = -1;
  vknvg_create_vertshader(fillVertShader, fillVertShaderSize);
  uint32_t* fillFragShader = nullptr; uint32_t fillFragShaderSize = -1;
  vknvg_create_fragshader(fillFragShader, fillFragShaderSize);
  uint32_t* fillFragShaderAA = nullptr; uint32_t fillFragShaderAASize = -1;
  vknvg_create_fragshader_aa(fillFragShaderAA, fillFragShaderAASize);

  vk->fillVertShader = vknvg_createShaderModule(device, fillVertShader, fillVertShaderSize, allocator);
  vk->fillFragShader = vknvg_createShaderModule(device, fillFragShader, fillFragShaderSize, allocator);
  vk->fillFragShaderAA = vknvg_createShaderModule(device, fillFragShaderAA, fillFragShaderAASize, allocator);
  ...
  vknvg_destroy_vertshader(fillVertShader);
  vknvg_destroy_fragshader(fillFragShader);
  vknvg_destroy_fragshader_aa(fillFragShaderAA);
  return 1;
}
...
static int vknvg_renderCreateTexture(void *uptr, int type, int w, int h, int imageFlags, const unsigned char *data) {
  ...
  VkFence fence;
  NVGVK_CHECK_RESULT(vkCreateFence(vk->createInfo.device, &fenceCI, nullptr, &fence));
  NVGVK_CHECK_RESULT(vkQueueSubmit(vk->createInfo.graphicsQueue, 1, &SI, fence));
